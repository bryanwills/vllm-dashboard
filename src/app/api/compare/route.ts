...
import { loadEvalRows, type EvalMetric, type EvalRow } from "@/lib/eval-data";
...
const PERF_METRICS: PerfMetricInfo[] = [
...
async function loadPerfRows({
  ...
  const conditions = [
    ...
    `message:image::STRING IN ('${escapeSqlString(baseline)}', '${escapeSqlString(candidate)}')`,
  ];
  if (model) {
    conditions.push(`message:model::STRING = '${escapeSqlString(model)}'`);
  }
  if (device) {
    conditions.push(`message:device::STRING = '${escapeSqlString(device)}'`);
  }
...
function sortDeltas(a: DeltaItem, b: DeltaItem): number {
