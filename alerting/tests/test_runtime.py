...
from alerting.ports import AutomationExecutionStatus
...
class CountingHandler:
    ...
    def __init__(self, error: Exception | None = None) -> None:
        self.calls: list[ScheduledCommand] = []
        self.error = error
...
def make_runtime(
    ...
    clock: FixedClock,
    executions: InMemoryAutomationExecutionStore | None = None,
) -> AlertingRuntime:
    return AlertingRuntime(
        executions=executions or InMemoryAutomationExecutionStore(),
...
def test_processing_same_command_twice_runs_handler_once() -> None:
    ...
    assert record.status is AutomationExecutionStatus.COMPLETED
...
def test_unknown_command_type_is_rejected_without_an_execution_record() -> None:
    ...
    executions = InMemoryAutomationExecutionStore()
    runtime = make_runtime(handler, FixedClock(START), executions)
    command = ScheduledCommand(command_type="mystery", target_time=START)
